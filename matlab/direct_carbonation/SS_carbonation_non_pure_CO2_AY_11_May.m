...
pCO2_in = 0.3; % CO2 concentration in tank, bar
...
MW_CaOH2 = 74.1;
MW_CaO = 56.08;
MCao = 0.3940; % Tian et al paper, 26.81% of Ca(OH)2; CaO content in steel slag

Mr = 0.7857; % Molar mass ratio of CO2 to CaO (no unit)

initial_residence_time = 0.0001; % Hours
...
y0 = [repmat([CCO2_in, X0], 1, n_com), m0, dE_dt_init]; % Initial values
...
filename = 'carbonation_summary_nonpure.xlsx';
summary_data = table(SS_total, CO2_max_stored, CO2_total_stored, ...
    CO2_total_stored/SS_total, carbonation_duration, ...
    CO2_total_supplied, fan_energy, CO2_total_supplied, ...
    CO2_total_stored/CO2max_l, pCO2_in, T, dp, ...
    'VariableNames', {'SS_total_mass_kg', 'CO2_max_stored_kg', ...
    'CO2_total_stored_kg', 'CO2_total_stored_per_kgSS', ...
    'carbonation_duration_hr', 'CO2_total_supplied_kg', ...
    'fan_energy_MJ', 'CO2_total_supplied_kg_dup', ...
    'carbonation_degree', 'pCO2_in_bar', 'T_K','dp_m'});
...
figure
plot(t/3600,y(:,2*n_com+1),'b');                  % CO2 captured
hold on;
...
% 文件名
filename = 'CO2_captured_data.xlsx';
...
%figure
plot(t/3600,y(:,2*n_com+2)/(1e6),'b');                  % Energy consuption 
hold on;
...
required_carbonation_degree = 0.9;
CO2_absorbed = y(2*n_com+1);
...
dydt = zeros(2*n_com+2, 1); % Since there is only 1 compartment, dydt length is 4
R = zeros(n_com,1);
Q_in = zeros(n_com,1);
Q_out = zeros(n_com,1);
...
if t > (recorded_time + control_interval)
    CCO2_out = y(2*(n_com-1)+1);
    if CCO2_out * Q_out_final >= 1e-2 * CCO2_in * Q_feed
        Q_feed = 0.9* Q_feed;
        control_interval = 10;
    end
...
delta_p = [];
for j = 1:n_com
    Cco2 = y(2*(j-1)+1); % CO2 concentration
    X = y(2*j); % Reaction conversion
 ...
 if j >1 
    ...
    dCco2_dt = (Q_in(j) * CCO2_in_j - Q_out(j)*Cco2 - R(j) / 0.044) / V_CO2;
...
dydt(2*(j-1)+1) = dCco2_dt;
dydt(2*(j-1)+2) = dX_dt; % Change in reaction progress
end
% Compute dCco2_dt
%dCco2_dt = (Q_feed * (CCO2_in - Cco2) - R / 0.044) / V_CO2;
global t_record t_ch_record t_Diff_record

% Record luelues at every step
t_record = [t_record; t];
...
P_out = P_in + delta_p_total;
Wg = (1 / eta) * P_in * Q_feed * gamma / (gamma-1) * ((P_out / P_in)^(1-1/gamma) - 1);

% Update ODE equations
dydt(2*n_com+1) = sum(R); % CO2 absorption amount
dydt(2*n_com+2) = Wg; % Fan power consumption
end
